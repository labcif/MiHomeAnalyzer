...
import logging, sys, os, traceback, time, datetime, threading, cv2, numpy
from . import config, motiondet, observer, observable
from . import videocapture, ffmpegwriter, pedestriandet, healthcheck
...
class videoloop(observer.observer, observable.observable):
    ...
    def __init__(self, fileName, mp4_files):
        ...
        if self.appConfig.camera['framePlugin'] == "codeferm.videocapture":
            self.framePluginInstance = self.getPlugin(moduleName="videocapture", url=self.appConfig.camera['url'])
    ...
    def getPlugin(self, moduleName, **kwargs):
        ...
        """
        #module = importlib.import_module("",package=None)
        if moduleName == 'videocapture':
            module = videocapture
        elif moduleName == 'ffmpegwriter':
            module = ffmpegwriter
        elif moduleName == 'pedestriandet':
            module = pedestriandet
        elif moduleName == 'healthcheck':
            module = healthcheck
    ...
    def makeFileName(self, timestamp, name):
        ...
        fileDir = "%s/%s" % (os.path.expanduser(self.appConfig.camera['recordDir']), self.appConfig.camera['name'])
        # Create dir if it doesn"t exist
        if not os.path.exists(fileDir):
            os.makedirs(fileDir)
        fileName = "%s %s.%s" % (name, datetime.datetime.fromtimestamp(timestamp).strftime("%Y-%m-%d %HH_%MM_%SS"), self.appConfig.camera['recordFileExt'])
    ...
    def recordingStart(self, timestamp, motionPercent):
        ...
        self.writerPluginInstance = self.getPlugin(moduleName='ffmpegwriter', fileName=self.videoFileName, vcodec=self.appConfig.camera['vcodec'], fps=self.fps, frameWidth=self.framePluginInstance.frameWidth, frameHeight=self.framePluginInstance.frameHeight)
        if self.appConfig.motion['historyImage']:
    ...
    def observeEvent(self, **kwargs):
        ...
        if kwargs["event"] == self.appConfig.motionStart:
            if not self.recording:
                video_in_list=self.fpsCount/60
                self.detects.append(video_in_list)
                video = self.mp4_files[floor(video_in_list)]
                timestamp_video = int(str(video).split('_')[-1].replace('.mp4', ''))
                if (video_in_list)%1 >=0.67:
                    timestamp_video+=2
                elif (video_in_list)%1 >= 0.34 and (video_in_list)%1 < 0.67:
                    timestamp_video+=1
                self.logger.debug("Motion start: %4.2f%%" % kwargs["motionPercent"])
                # Kick off recordingStart thread
                self.motion_file_name[datetime.datetime.fromtimestamp(timestamp_video).strftime("%Y-%m-%d %HH_%MM_%SS")]=video
                recordingStartThread = threading.Thread(target=self.recordingStart, args=(timestamp_video, kwargs["motionPercent"],))
        ...
        try:
            ...
            if self.framePluginInstance.frameWidth > 0 and self.framePluginInstance.frameHeight > 0:
                ...
                if self.appConfig.camera['detectPlugin'] != "":
                    ...
                    self.detectPluginInstance = self.getPlugin(moduleName='pedestriandet', appConfig=self.appConfig, image=self.frameBuf[0][0], logger=self.logger)
                    # Observe motion events
                ...
                if self.appConfig.camera['videoloopPlugins'] is not None:
                    ...
                    for item in self.appConfig.camera['videoloopPlugins']:
                        ...
                        pluginInstance = self.getPlugin(moduleName='healthcheck', appConfig=self.appConfig, logger=self.logger)
                        # Observe videoloop events
                        ...
                        if self.appConfig.camera['detectPlugin'] != "":
                            ...
                            if len(foundLocationsList) > 0 and self.recording:
                                ...
                                if self.appConfig.camera['saveFrames']:
                                    thread = threading.Thread(target=self.saveFrame, args=(frame, "%s/frames/%d.jpg" % (os.path.splitext(self.videoFileName)[0], self.recFrameNum)),)
                                    thread.start()
                    else:
                        skipCount -= 1
